...
@interface SLDrawerMenuVC () <UITableViewDelegate, UITableViewDataSource>
@end

@implementation SLDrawerMenuVC
...
- (void)setSelectedControllerIndex:(int)selectedControllerIndex animated:(BOOL)animated
{
	if (selectedControllerIndex != _selectedControllerIndex && selectedControllerIndex < self.menuItems.count) {
		...
		[self.SLDrawerRootVC setPaneViewController:selectedController animated:animated completion:nil];
	}

	[self.SLDrawerRootVC setPaneState:MSDynamicsDrawerPaneStateClosed animated:YES allowUserInterruption:YES completion:^{
